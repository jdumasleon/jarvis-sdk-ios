                        ...
                        if isSelectable {
                            Button(action: {
                                ...
                                Image(systemName: selectedRows.contains(row.id) ? "checkmark.square.fill" : "square")
                                    .font(.system(size: DSIconSize.m))
                                    .foregroundColor(selectedRows.contains(row.id) ? DSColor.Primary.primary100 : DSColor.Text.tertiary)
                            }
                        }
                ...
                Section("List Rows") {
                    VStack(spacing: DSSpacing.none) {
                        DSListRow(.init(
                            ...
                            trailingView: AnyView(Text("1.2KB").setTextStyle(.labelSmall)),
                            action: { }
                        ))

                        Divider().background(DSColor.Surface.divider)

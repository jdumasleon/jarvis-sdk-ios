    ...
    public var body: some View {
        ...
        .overlay(
            Rectangle()
                ...
                .foregroundColor(DSColor.Surface.border),
            alignment: .bottom
        )
    }
}
        ...
        Button(action: action) {
            VStack(spacing: DSSpacing.xxs) {
                ZStack {
                    ...
                    .foregroundColor(isSelected ? DSColor.Primary.primary100 : DSColor.Text.tertiary)
            ...
            ForEach(Array(items.enumerated()), id: \.element.id) { index, item in
                if let action = item.action {
                    Button(action: action) {
                        Text(item.title)
                            .setTextStyle(.labelSmall)
                            .foregroundColor(DSColor.Text.link)
                    }
                } else {
                    Text(item.title)
                        .setTextStyle(.labelSmall)
                ...
                if index < items.count - 1 {
                    DSIcons.Navigation.forward
                        ...
                        .foregroundColor(DSColor.Text.tertiary)
                }
            }
...
public struct DSSegmentedControl: View {
    ...
    private func backgroundColor(for segment: Segment) -> Color {
        selectedSegment == segment.id ? DSColor.Surface.surface : Color.clear
    }

    private func foregroundColor(for segment: Segment) -> Color {
        selectedSegment == segment.id ? DSColor.Text.primary : DSColor.Text.secondary
    }
}
            ...
            ForEach(visiblePageRange, id: \.self) { page in
                if page == -1 {
                    ...
                    Button(action: {
                        ...
                        Text("\(page)")
                            .setTextStyle(.labelSmall)
                            .foregroundColor(currentPage == page ? DSColor.Text.onPrimary : DSColor.Text.primary)
                            .frame(width: 32, height: 32)
...
public struct DSStepIndicator: View {
                    ...
                    ZStack {
                        ...
                        if step.isCompleted {
                            DSIcons.Status.success
                                .font(.system(size: DSIconSize.s))
                                .foregroundColor(DSColor.Text.onPrimary)
                        } else {
                            Text("\(index + 1)")
                                .setTextStyle(.labelSmall)
    ...
    private func circleBackgroundColor(for index: Int) -> Color {
        if steps[index].isCompleted {
            ...
            return DSColor.Neutral.neutral300
        }
    }
